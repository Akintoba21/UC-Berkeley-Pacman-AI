...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if state[0] in self.corners:
            if state[0] not in cornersVisited:
...
def cornersHeuristic(state, problem):
    ...
    currentCalced = currentCorners[:]
    dist = 0
    while not len(currentCalced) == 4:
        minDist = None
        minCorn = None
        for i in corners:
            if i not in currentCalced:
                curDist = manhattanDistance(current, i)
                if minDist == None or curDist < minDist: 
                    minDist = curDist
                    minCorn = i
        dist += minDist
        currentCalced.append(minCorn)
        current = minCorn
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    #looping through foodGrid is far too slow but this works
