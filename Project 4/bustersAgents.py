...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for p in livingGhostPositionDistributions:
            dist = self.distancer.getDistance(pacmanPosition, p.argMax())
            if minGhost is None or dist < minGhost: 
                minGhost = dist
                minGhostPos = p.argMax()
        action = None
        adist = None
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition,a)
            dist = self.distancer.getDistance(successorPosition, minGhostPos)
            if adist is None or dist < adist:
                action = a
                adist = dist
        return action
